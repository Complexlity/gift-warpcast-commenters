...
if (!url) {
  throw new Error('Pass the cast url as an argument. e.g node getReplies.js your_cast_url')
}
...
// fs.writeFileSync('cast.json', JSON.stringify(data2.result.casts))
	...
	const repliersAddresses = convertRepliesToAddressArray(data2.result.casts, excludeThreadOwner)
...
function convertRepliesToAddressArray(casts, excludeThreadOwner) {
  ...
  casts.forEach((cast) => {
    ...
    if (castAuthorVerificationAddress != threadOwnerVerificationAddress && castAuthorVerificationAddress) {
